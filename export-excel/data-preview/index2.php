      ...
      <div class="panel-header">
        ...
        <div style="display:flex;gap:7px;align-items:center">
          ...
          <button class="ld-btn ld-btn-green" id="btn-download-csv" disabled>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> Download CSV
          </button>
        </div>
      </div>
...
document.getElementById('btn-download-csv').addEventListener('click',function(){
  ...
  var SEP=';',lines=[];
  lines.push(ce('Reporte Data Preview \u2014 Zabbix Report'));
  var meta=[ce('Generado:'),ce(ts)];
  if(from)meta.push(ce('Rango:'),ce(from+' \u2192 '+to));
  lines.push(meta.join(SEP));
  lines.push(ce('Hosts:')+SEP+ce(sh.length)+SEP+ce('Columnas:')+SEP+ce(sc.length));
  lines.push('');
  lines.push([ce('Host')].concat(sc.map(ce)).join(SEP));
  buildRows(sh,sc).forEach(function(row){
    lines.push([ce(row.host)].concat(sc.map(function(col){var v=row.cells[col];return ce(v==='—'||v==='\u2014'?'':v||'');})).join(SEP));
  });
  var ds=now.getFullYear()+''+pad(now.getMonth()+1)+''+pad(now.getDate());
  var blob=new Blob(['\uFEFF'+lines.join('\r\n')],{type:'text/csv;charset=utf-8;'});
  var url=URL.createObjectURL(blob),a=document.createElement('a');
  a.href=url;a.download='zabbix_inventory_'+ds+'.csv';
